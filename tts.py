...
USE_VERBOSE = False
MAX_TTS_CHARACTERS = 5000  # Limit for TTS audio generation
...
async def generate_tts_audio(text: str, output_file_path: str = None, play_audio: bool = False) -> str:
    """Generate TTS audio from text and save to file."""
    try:
        if not output_file_path:
            output_file_path = os.path.join(tempfile.gettempdir(), f"tts_output_{int(time.time())}.mp3")
        ...
        # Limit the text length to avoid excessive processing time
        text = text[:MAX_TTS_CHARACTERS] if len(text) > MAX_TTS_CHARACTERS else text
        
        # Save to file
        engine.save_to_file(text, output_file_path)
        engine.runAndWait()
        
        # Convert to MP3 if not already (pyttsx3 may save as WAV on some systems)
        if not output_file_path.endswith('.mp3'):
            sound = AudioSegment.from_file(output_file_path)
            mp3_path = os.path.splitext(output_file_path)[0] + '.mp3'
            sound.export(mp3_path, format="mp3")
            os.remove(output_file_path)
            output_file_path = mp3_path
        
        logging.info(f"TTS audio saved to: {output_file_path}")
...
async def main():
    try:
        ...
        tts_output_path = base_name + '_llm_corrected_tts.mp3'
        ...
        # Generate TTS audio for the first part of the document
        tts_text = cleaned_text[:MAX_TTS_CHARACTERS]
        tts_file = await generate_tts_audio(tts_text, tts_output_path)
        if tts_file:
            logging.info(f"TTS audio generated and saved to: {tts_file}")
        else:
            logging.warning("Failed to generate TTS audio")
        ...
        if quality_score is not None:
            ...
            # Generate TTS for the quality assessment
            assessment_tts = f"Quality assessment complete. The document received a score of {quality_score} out of 100. {explanation}"
            assessment_tts_path = base_name + '_quality_assessment.mp3'
            await generate_tts_audio(assessment_tts, assessment_tts_path)
        else:
        ...
        if tts_file:
            logging.info(f" TTS Audio: {tts_file}")
        if quality_score is not None:
            logging.info(f" Quality Assessment Audio: {assessment_tts_path}")
    except Exception as e:
